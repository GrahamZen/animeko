...
* 电视上终于能在播放器里**发表本集评论**了（以前只能回复别人的）：进度条上方那颗「评论」按下确定即可，弹出输入框直接打字。
* 电视上写评论可以插表情了：输入框下面多了「添加表情」，左边选表情包、右边挑一枚，六个官方表情包加颜文字都能用。旁边的「预览」可以先看看发出去长什么样。
...
* 电视上的卡片导航改成「聚焦框不动、卡片滑动」：探索页、详情页与播放器的选集卡片，聚焦框都钉在固定位置，上下左右导航只有卡片在框下滑过。滑出屏幕左缘、上缘的卡片不再被直角切断，而是压暗、渐隐着滑走；「继续观看」「推荐」这类区块标题跟着卡片一起移动，到位后停在原处不再乱跳。
* 电视上长按方向键不会越按越快了：卡片按固定节奏移动，看得清当前停在哪一张（横向、纵向各有一个上限，换行比换卡慢一档）。
