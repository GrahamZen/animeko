...
这一版集中收拾电视端界面的性能，另外没有横版背景图的作品不再是一片黑底。

> 补充：刚装完的头几次打开会比较卡，这是正常的。系统需要在后台把应用预编译成机器码，期间只能边跑边翻译，速度差好几倍。放着不管，用上几次或过一晚就会自己变流畅，之后一直保持——所以刚装上觉得卡，先多用两次再判断。

### 界面

* 没有横版背景图的作品，首页、追番、搜索、时间表的背景改用竖版封面顶上（和详情页一样），不再是纯黑。
* 优化进详情页的加载速度。
* 优化卡片背景的加载速度。
* 微调所有卡片的外观以及聚焦效果。
...
* 开着「完整视觉效果」时，首页卡片还在加载的那段时间整页明显变卡。
* 遥控器移动焦点时，卡片的聚焦描边不再让整张卡重新排版。
* 播放器控制层的淡入淡出，以及「显示跳过按钮」档下整段 OP/ED 期间看不见的绘制。
* 滚动表情包时，弹窗正文和所有带表情的评论行会跟着整体重排。
...
* 打开评论要先干等最多 3 秒才出第一条：那是在等「回复了谁」这层信息，现在等不到就先把评论放出来。
