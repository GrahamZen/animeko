...
data class SubjectDetailsLayoutParams(
    ...
    companion object {
         ...
         */
        val Tv = SubjectDetailsLayoutParams(
            kind = SubjectDetailsPaneKind.MEDIUM,
            sidebarWidth = 220.dp,
            ...
            railItemSpacing = 0.dp,
            columnSpacing = 24.dp,
            // 10-foot UI 留白偏大 (Prime Video 实测约 75dp); 48dp 兼顾 TV 安全区与内容宽度
         ...
         */
        fun calculate(availableWidth: Dp, isTv: Boolean = false): SubjectDetailsLayoutParams = when {
            // TV 屏幕高度小 (通常 ~540dp), 手机/平板档的侧栏封面会占满整屏, 用专属参数档. 见 [Tv].
            isTv && availableWidth >= MEDIUM_WIDTH_DP.dp -> Tv
