...
fun FocusEpisodeCarousel(
    ...
    LaunchedEffect(actionRestoreEpisodeId) {
        ...
        // 独立窗口关闭后先等宿主窗口真正重新获得焦点，再单次送往目标卡.
        snapshotFlow { windowInfo.isWindowFocused }.first { it }
        if (activeFocusEpisodeId == target) actionRestoreEpisodeId = null
        else focusScope.request(EpisodeCarouselFocus(target))
    }
    LaunchedEffect(focusScope.userNavGeneration) {
        if (actionRestoreEpisodeId != null) actionRestoreEpisodeId = null
    }
                                ...
                                modifier = Modifier.width(cellWidth)
                                    ...
                                    .then(
                                        if (isActionRestoreTarget) {
                                            Modifier.tvFocusAnchor(
                                                focusScope,
                                                EpisodeCarouselFocus(item.episodeId),
                                            )
                                        } else Modifier,
                                    )
                                    .then(
                                    ...
                                    .onFocusChanged {
                                        if (it.isFocused) {
                                            ...
                                            dimPivotIndex = index
                                            if (actionRestoreEpisodeId == item.episodeId) {
...
private data class EpisodeCarouselFocus(val episodeId: Int) : TvFocusKey
