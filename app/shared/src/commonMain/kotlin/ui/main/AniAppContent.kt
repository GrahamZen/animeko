...
fun AniAppContent(aniNavigator: AniNavigator) {
    ...
    // TV: 根底色恒黑 —— 页面切换过渡的淡入淡出间隙会露出根底色, 浅色主题下白色一闪
    // 很刺眼 (尤其播放器 → 缓存页, 前后都是暗色内容); 各页面都自绘不透明背景, 根底色
    // 只在过渡间隙可见, 黑色对 TV 全屏形态最自然.
    val rootBackground =
        if (LocalPlatform.current.isTv()) Color.Black else MaterialTheme.colorScheme.background
