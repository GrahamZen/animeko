...
internal fun EpisodeVideoImpl(
    ...
    // When any side sheet closes (via item selection, X button, or Back key), restore focus
    // to the button that opened it. The Back key handler also does this directly, but sets
    // lastSideSheetFocusRequester = null first, so this LaunchedEffect becomes a no-op for
    // that path and only handles the onDismissRequest / item-selection path.
    LaunchedEffect(anySideSheetVisible) {
        if (!anySideSheetVisible && isTv) {
            kotlinx.coroutines.delay(FOCUS_REQ_DELAY_MILLIS)
    ...
    // Hoisted state for finding seek/skip controller (TV 按键处理与手势层共用同一 indicatorState)
    ...
    AniTheme(darkModeOverride = DarkMode.DARK) {
        ...
        VideoScaffold(
            ...
            modifier = Modifier
                ...
                .focusable()
                .ifThen(isTv) {
                    // 上游 #3137 后播放器表面的焦点在手势宿主的 clickable 上, 确认键被它
                    // 当点击消费, 不会再冒泡到下面 onKeyEvent 的 Enter/DirectionCenter 分支.
                    // 在 preview 阶段先拦下确认键, 恢复"确认唤出面板并聚焦弹幕设置按钮";
                    // 焦点在控制条按钮上或侧边 sheet 打开时不拦截, 保持按钮正常点击
                    onPreviewKeyEvent { keyEvent ->
                        if (keyEvent.type == KeyEventType.KeyDown &&
                            (keyEvent.key == Key.Enter || keyEvent.key == Key.DirectionCenter) &&
                            !anyButtonHasFocus && !anySideSheetVisible
                        ) {
                    ...
                    PlayerTopBar(
                        ...
                        actions = {
                            EpisodeVideoTopBarActions(
                                ...
                                mediaSelectorFocusRequester = mediaSelectorFocusRequester,
                                settingsFocusRequester = settingsFocusRequester,
                                sidebarFocusRequester = sidebarFocusRequester,
                                onSheetNavigate = { lastSideSheetFocusRequester = it },
                            )
                        },
...
private fun EpisodeVideoTopBarActions(
    ...
    mediaSelectorFocusRequester: FocusRequester,
    settingsFocusRequester: FocusRequester,
    sidebarFocusRequester: FocusRequester,
    onSheetNavigate: (FocusRequester) -> Unit,
) {
    ...
    if (expanded) {
        IconButton(
            {
                onSheetNavigate(mediaSelectorFocusRequester)
                sheetsController.navigateTo(EpisodeVideoSideSheetPage.MEDIA_SELECTOR)
            },
            Modifier.testTag(TAG_SHOW_MEDIA_SELECTOR).focusRequester(mediaSelectorFocusRequester),
        ) {
    ...
    IconButton(
        {
            onSheetNavigate(settingsFocusRequester)
