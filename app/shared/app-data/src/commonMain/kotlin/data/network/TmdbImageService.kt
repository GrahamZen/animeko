...
class TmdbImageService(
     ...
     */
    suspend fun getAllBackdropUrls(subjectId: Int, originalName: String): List<String> = withContext(ioDispatcher) {
        if (currentAniBuildConfig.tmdbApiToken.isBlank() || originalName.isBlank()) return@withContext emptyList()

        readCache().allBackdrops[subjectId]?.let { return@withContext it }
        ...
        dataStore.updateData {
            it.copy(allBackdrops = it.allBackdrops + (subjectId to urls))
        }
        urls
    }
    ...
    private companion object {
         ...
         * (探索/追番页拿它当全屏 hero 背景, 需要原图); 选集卡片消费端用
         * [tmdbStillCardSizeUrl] 降到 w780 档.
         */
