...
data class NavigationMotionScheme(
    ...
    companion object {
        ...
        /**
         * TV: 全屏页面间的同步 crossfade (dissolve, 无位移): 上一页变浅的同时下一页变深,
         * 同时长同步进行, 中途不经过空白底色 (fade-through 的"先全白再显示"在全屏
         * 海报页之间观感突兀). 线性缓动保证从第一帧就开始渐变.
         */
        fun calculateTv(): NavigationMotionScheme {
            val exit = fadeOut(tween(TV_CROSSFADE_DURATION, easing = LinearEasing))
            val enter = fadeIn(tween(TV_CROSSFADE_DURATION, easing = LinearEasing))
        ...
        private const val TV_CROSSFADE_DURATION = 1000
