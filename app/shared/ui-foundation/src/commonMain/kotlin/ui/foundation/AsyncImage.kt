...
internal fun ImageRequest.Builder.configureAniImageRequest(
    ...
    scale(aniCropScale(contentScale, alignment))
    when (contentScale) {
 ...
 * 裁剪保留哪一侧.
 *
 * **不能直接用 sketch 的 `toScale()`: 它忽略 alignment** —— 实测 `Crop` 配 TopCenter / Center /
 * BottomCenter 一律返回 `CENTER_CROP`. 而 [ContentScale.Crop] 的裁剪发生在**解码时**
 * (见 [configureAniImageRequest] 给它配的 `Precision.SAME_ASPECT_RATIO`), 所以"顶部对齐裁剪"
 * 一旦退化成中心裁剪, 脸在解码那一刻就被丢掉了, Compose 侧的 alignment 再也救不回来.
 *
 * 具体症状: 角色/制作人员的圆形头像用的是全身立绘 (脸在顶部), 中心裁剪后只剩身体.
 *
 * sketch 的 [Scale] 只有一维 (START/CENTER/END, 作用在被裁掉的那个方向上), 所以这里只按
 * alignment 的**纵向**偏置决定: 立绘是竖图裁成方形, 被裁的正是高度. 横图 (backdrop 之类) 配
 * 非居中 alignment 时会变成"保留左/右", 但 fork 里那些都用居中, 走的还是原来的路径.
 */
internal fun aniCropScale(contentScale: ContentScale, alignment: Alignment): Scale {
    val verticalBias = (alignment as? BiasAlignment)?.verticalBias
        ?: return toScale(contentScale, alignment)
    return when {
        verticalBias < 0f -> Scale.START_CROP // 顶部对齐: 保留顶部 (立绘的脸)
        verticalBias > 0f -> Scale.END_CROP
        else -> toScale(contentScale, alignment)
    }
}
