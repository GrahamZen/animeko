...
internal fun releaseApkAssets(
    version: String,
    prefix: String = "ani",
    repository: String = "$FORK_OWNER/$FORK_REPO",
): List<GitHubAsset> = RELEASE_APK_SUFFIXES.map { suffix ->
    val name = "$prefix-$version-$suffix.apk"
    GitHubAsset(name, "https://github.com/$repository/releases/download/v$version/$name")
}
...
class UpdateChecker(private val client: ScopedHttpClient) {
    ...
    /**
     * 跳板包要装的新应用: 固定是它的首个正式版 (`AniBuildConfig.migrationLandingVersion`), 不找最新版 ——
     * 只有那一版带着接管旧包数据的代码, 之后的版本不再管迁移. 接管完照常更新到最新版.
     *
    ...
    private suspend fun getMigrationLanding(): NewVersion? {
        ...
        val release = try {
            client.use {
                get("https://api.github.com/repos/${currentAniBuildConfig.updateRepository}/releases/tags/v$version") {
                    header(HttpHeaders.UserAgent, getAniUserAgent())
                    mirrorTimeout()
                }.bodyAsText()
        ...
        val assets = release?.assets
            ...
            ?: releaseApkAssets(version, currentAniBuildConfig.updateAssetPrefix, currentAniBuildConfig.updateRepository)
