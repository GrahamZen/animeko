...
internal const val TV_SORT_LONG_PRESS_REPEATS = 2
 ...
 */
@Composable
internal fun SettingsScope.TvSortMediaSourceList(
                        ...
                        .onPreviewKeyEvent { event ->
                            ...
                            when {
                                isConfirmKey && event.type == KeyEventType.KeyDown -> {
                                    confirmKeyDownCount++
                                    // 按住到阈值立即选中 (不等松开)
                                    if (!longPressFired && confirmKeyDownCount >= TV_SORT_LONG_PRESS_REPEATS) {
                                        longPressFired = true
