            ...
            if (next != slow) {
                ...
                logger.info {
                    "Image network tier -> ${if (next) "SLOW" else "FAST"} " +
                            ...
                            "hedge=${tvBackdropHedgeMillis()}ms " +
...
/**
 * 显示中的背景图超过这么久没上屏就再叠一张同 URL 的请求 (不取消原来那条), 见 `TvBackdropImage`.
 *
 * 快档 4s / 慢档 12s. 慢档不能直接关掉: 请求真挂死时这是唯一的活路; 但也不能用 4s ——
 * 慢网上 4s 只是"还在下", 那一发纯粹在抢带宽. 12s 只在真出事时才轮得到.
 */
fun tvBackdropHedgeMillis(): Long = when (TvImageNetworkSpeed.tier) {
    TvImageNetworkTier.FAST -> 4_000L
    TvImageNetworkTier.SLOW -> 12_000L
}
