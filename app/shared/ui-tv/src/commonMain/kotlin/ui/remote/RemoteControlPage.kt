...
/* 数据源列表顶部的工具行 ("全部测试") */
.src-tools { display: flex; flex-wrap: wrap; align-items: center; gap: 10px; margin: 0 2px 12px; }
...
/* 数据源列表末尾的「重新搜索(含新数据源)」: 不抢眼, 但找得到 */
.src-refetch { margin: 14px 2px 4px; }
...
(function () {
  ...
  function renderList(s) {
    ...
    // 这一次搜索用的是进播放页那一刻的数据源列表 (会话建立时取的快照), 之后新增 / 启用的源、更新过的订阅都不在里面。
    // 把重刷的入口放在列表末尾: 发现"刚加的源没出现"正是在这儿。
    h += '<div class="src-refetch"><button type="button" id="src-refetch">' + T('重新搜索（含新数据源）') + '</button>' +
...
(function () {
  ...
  function renderList() {
    ...
    box.innerHTML = '<div class="src-tools"><button type="button" id="src-test-all">' + T('全部测试') + '</button>' +
      ...
      b += '<button data-act="test" class="ic">' + I.plug + T('测试') + '</button>';
  ...
  function testSource(item, s, btn) {
    ...
    return post('api/sources/test', { id: s.id })
      ...
      .then(function (ok) { btn.disabled = false; btn.innerHTML = label; return ok; });
  }
  /*
   * 「全部测试」: 一条条点太累 (二十几个源)。同时发出去, 由浏览器自己限同域并发 —— 不在服务端并发,
   * 那样一拥而上反而容易把慢站和要验证码的站逼出假失败 (电视上的连通性探测踩过, 见 memory
   * project-connectivity-probe-false-red)。结果各自落到自己那一行, 按钮上只报进度。
   */
  function testAllSources(btn) {
    var items = box.querySelectorAll('.src-item');
    var jobs = [];
    for (var i = 0; i < items.length; i++) {
      var it = items[i];
      var s = data.sources[+it.getAttribute('data-i')];
      var b = it.querySelector('button[data-act="test"]');
      if (s && b && !b.disabled) jobs.push(testSource(it, s, b));
    }
    if (!jobs.length) return;
    var done = 0, okCount = 0;
    btn.disabled = true;
    function tick() { btn.textContent = T('测试中 {0}/{1}', done, jobs.length); }
    tick();
    jobs.forEach(function (p) {
      p.then(function (ok) { done++; if (ok) okCount++; tick(); });
    });
    Promise.all(jobs).then(function () {
      btn.disabled = false;
      btn.textContent = T('全部测试');
      toast(T('测试完成：{0} 个正常，{1} 个有问题', okCount, jobs.length - okCount));
    });
  }
  ...
  box.addEventListener('click', function (e) {
    var all = e.target.closest('#src-test-all');
    if (all) { testAllSources(all); return; }
